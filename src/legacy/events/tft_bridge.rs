...
impl From<pallet_tft_bridge::Event<runtime_legacy::Runtime>> for Event {
    fn from(tbe: pallet_tft_bridge::Event<runtime_legacy::Runtime>) -> Self {
        match tbe {
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::MintTransactionProposed(
                tx_id,
                target,
                amount,
            ) => Event::MintTransactionProposed(tx_id, target, amount.into()),
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::MintTransactionVoted(tx_id) => {
                Event::MintTransactionVoted(tx_id)
            }
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::MintCompleted(tx) => {
                Event::MintCompleted(tx.into())
            }
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::MintTransactionExpired(
                tx_id,
                amount,
                target,
            ) => Event::MintTransactionExpired(tx_id, amount.into(), target),
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::BurnTransactionCreated(
                burn_id,
                stellar_address,
                amount,
            ) => Event::BurnTransactionCreated(burn_id, stellar_address, amount.into()),
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::BurnTransactionProposed(
                tx_id,
                target,
                amount,
            ) => Event::BurnTransactionProposed(tx_id, target, amount.into()),
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::BurnTransactionSignatureAdded(
                tx_id,
                sig,
            ) => Event::BurnTransactionSignatureAdded(tx_id, sig.into()),
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::BurnTransactionReady(tx_id) => {
                Event::BurnTransactionReady(tx_id)
            }
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::BurnTransactionProcessed(tx) => {
                Event::BurnTransactionProcessed(tx.into())
            }
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::BurnTransactionExpired(
                tx_id,
                target,
                amount,
            ) => Event::BurnTransactionExpired(tx_id, target, amount.into()),
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::RefundTransactionCreated(
                tx_hash,
                target,
                amount,
            ) => Event::RefundTransactionCreated(tx_hash, target, amount.into()),
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::RefundTransactionsignatureAdded(
                tx_hash,
                sig,
            ) => Event::RefundTransactionsignatureAdded(tx_hash, sig.into()),
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::RefundTransactionReady(tx_hash) => {
                Event::RefundTransactionReady(tx_hash)
            }
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::RefundTransactionProcessed(tx) => {
                Event::RefundTransactionProcessed(tx.into())
            }
            pallet_tft_bridge::Event::<runtime_legacy::Runtime>::RefundTransactionExpired(
                tx_id,
                target,
                amount,
